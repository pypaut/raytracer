...
{
    public:
        Image(std::vector<Pixel> pixels, int W, int H)
        {
            pixels_ = pixels;
            W_ = W;
            H_ = H;
        ...
        std::vector<Pixels> get_pixels()
        {
            return pixels_;
        }

        std::ostream& operator<<(std::ostream &out, Image &im)
        {
            int i = 0;
            auto pixels = im.get_pixels();

            for (auto it pixels.begin(); it != pixels.end(); it++, i++) {
                std::string end = "";
                if (i > im.get_W()) {
                    end = ",\n";
                    i %= W_;
